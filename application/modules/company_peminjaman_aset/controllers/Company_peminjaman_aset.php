...
class Company_peminjaman_aset extends MX_Controller {
	...
	*/
    ...
    {  
	    ...
	    $data = $this->Company_information_model->adddataproses_peminjaman_aset($no_unit, $status, $dipakai_oleh, $jam_booking, $dengan_tujuan, $akan_tersedia_pada_pukul);
    ...
    {  
	    ...
	    $data = $this->Company_information_model->editdataproses_peminjaman_aset($id, $no_unit, $status, $dipakai_oleh, $jam_booking, $dengan_tujuan, $akan_tersedia_pada_pukul);
		echo json_encode($data);
    }
    ...
    { 
		$id = $_POST['id']; 
		$data = $this->Company_information_model->removedata_peminjaman_aset($id); 
	...
	public function getdata()
	{
		...
		$data = $this->Company_information_model->getdata_peminjaman_aset();
		echo json_encode($data);
	...
	{
		$this->load->view('commons/commons_from_gentelella_ocs/header');
		$this->load->view('company_peminjaman_aset');
		$this->load->view('commons/commons_from_gentelella_ocs/footer');
	}
	
	public function unggahexcel() {
        //ambil data form
        $file = $_FILES['company_peminjaman_aset_name']['tmp_name'];
            ...
            foreach ($value as $item) {
				switch($y){
					...
					case 5:
						...
						break;
					case 6:
						$field = "akan_tersedia_pada_pukul";
						break;
        ...
        foreach ($emp as $value) {
			$query = $this->DB->insert('peminjaman_aset', $value);
        }
