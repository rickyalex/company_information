...
<!--no_unit, status, dipakai_oleh, jam_booking, dengan_tujuan, akan_tersedia_pada_pukul-->
				<div class="col-md-12 col-sm-12 col-xs-12 bg-white">
					<div id="getdata_peminjaman_aset">
						<table id="table" 
							data-toggle="table"
							data-search="true"
...
<script = type="text/javascript">
	...
	function actionFormatter(value, row, index) {
    	...
    	var id = objek.id;
				return [
    				...
    				'</a>    ',
    				'<a class="edit btn btn-info btn-xs" href="'+base_url+'company_peminjaman_aset/editdata/'+id+'" title="Ubah">',
	...
	window.actionEvents = {
        ...
        'click .remove': function (e, value, row, index) {
          //  alert('delete: ' + JSON.stringify(row));
			...
			//alert(id);
     	    $.ajax({
    			  url: '<?php echo base_url();?>company_peminjaman_aset/removedata/',
    			  dataType: 'json',
    			  type: 'POST',
    			  ...
    			  success: function (data){
    				  if(data == true){
                          ...
                          location.reload();
    					  $('#getdata_peminjaman_aset').load('company_peminjaman_aset');
    				  }
    			  }
    			}); 
            console.log(value, row, index);
	...
	//form submit menggunakan FormData harus menggunakan browser versi IE 10+, Firefox 4.0+, Chrome 7+, Safari 5+, Opera 12+
			jQuery("form#company_peminjaman_aset").submit(function (event) {
				...
				jQuery.ajax({
					...
					contentType: false,
					processData: false,
					url: "<?php echo base_url(); ?>company_peminjaman_aset/unggahexcel/",
